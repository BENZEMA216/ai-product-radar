...
- 让用户能在网页上快速反馈“值得看 / 不该收录 / 应该降权 / 漏掉产品 / 我的点评”。
...
- 如果同一个 Pacific 完成日榜已经在早前手动/正式报告中发布，日报会对 PH 产品链接做历史去重；source health 中 `keptCount` 继续表示本次发现的 AI 相关 PH 候选数，`reportKeptCount` 表示历史去重后实际进入本次报告的 PH 条数，避免把“抓到了但去重未发布”误判成来源失败。
